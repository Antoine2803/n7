...
%
    ...
    A0 = beta(1);
    lambda = beta(2);
    res = donnees(:,2) - A0*exp(-lambda*donnees(:,1));
...
%
    % TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dr_beta1 = -exp(-beta(2)*donnees(:,1));
    dr_beta2 = beta(1)*donnees(:,1).*exp(-beta(2)*donnees(:,1));
    J_res = [dr_beta1,dr_beta2];
...
%
    % TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Coeff11 = sum(exp(-2*beta(2).*donnees(:,1)));
    Coeff12 = sum((donnees(:,2).*donnees(:,1)).*exp(-beta(2).*donnees(:,1)) - 2*beta(1).*donnees(:,1).*exp(-2*beta(2).*donnees(:,1)));
    Coeff22 = sum(-donnees(:,2).*beta(1).*donnees(:,1).*donnees(:,1).*exp(-beta(2).*donnees(:,1)) + 2*beta(1).*beta(1).*donnees(:,1).*donnees(:,1).*exp(-2*beta(2).*donnees(:,1)));
    
    res = residu(beta);
    J_res = J_residu(beta);
    H_f = [Coeff11, Coeff12; Coeff12, Coeff22];
