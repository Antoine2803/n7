...
function [valeur_coupure,ind_variable_coupure] = premiere_coupure(X,Y)
    ...
    for k = 1:size(X,2)
        ...
        for split = 1:size(X,1)-1
            ...
            %%% A COMPLETER
            Gini_g = 1 - (sum(Y_g == 1)/split + sum(Y_g == -1)/split)^2;
            Gini_d = 1 - (sum(Y_d == 1)/(n-split) + sum(Y_d == -1)/(n-split))^2;
            % Calcul de delta_i_modifie = p_g Gini_g + p_d Gini_d
            delta_i_modifie = (sum(Y == -1)/length(Y)) * Gini_g + (sum(Y == 1)/length(Y)) * Gini_d;
